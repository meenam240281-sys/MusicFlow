...
const EmbedManager = {
    ...
    // Create Spotify player with proper embed - UPDATED FOR BETTER URL HANDLING
    createSpotifyPlayer(container, url) {
        if (!url) {
            container.innerHTML = `
                <div class="player-error">
                    <i class="fab fa-spotify"></i>
                    <h3>No Spotify URL</h3>
                    <p>Please enter a Spotify playlist, track, or album URL.</p>
                </div>
            `;
            return;
        }
        ...
        if (!spotifyUri) {
            container.innerHTML = `
                <div class="player-error">
                    <i class="fab fa-spotify"></i>
                    <h3>Could Not Process Spotify URL</h3>
                    <p>Please try one of these formats:</p>
                    <div style="text-align: left; margin-top: 15px; font-size: 0.9rem;">
                        <p><strong>Full URL:</strong><br>
                        https://open.spotify.com/playlist/37i9dQZF1DXcBWIGoYBM5M</p>
                        
                        <p><strong>URL with parameters:</strong><br>
                        https://open.spotify.com/playlist/0ndqpswfcMZCV9z01OQ9Qp?si=abc123</p>
                        
                        <p><strong>Spotify URI:</strong><br>
                        spotify:playlist:37i9dQZF1DXcBWIGoYBM5M</p>
                    </div>
                    <button class="btn-secondary" id="try-again-btn" style="margin-top: 20px;">
        ...
        container.innerHTML = `
            ...
            <div class="player-controls">
                ...
                <button class="btn-secondary" id="copy-embed-link" title="Copy embed URL to clipboard">
                    <i class="fas fa-link"></i> Copy Embed URL
                </button>
            </div>
        `;
        ...
        // Add copy embed URL button
        const copyBtn = document.getElementById('copy-embed-link');
        if (copyBtn) {
            copyBtn.addEventListener('click', () => {
                navigator.clipboard.writeText(embedUrl)
                    .then(() => {
                        ...
                        setTimeout(() => {
                            copyBtn.innerHTML = '<i class="fas fa-link"></i> Copy Embed URL';
                        }, 2000);
                    })
                    .catch(err => {
    ...
    // URL validation helpers - UPDATED FOR BETTER SPOTIFY SUPPORT
    validateSpotifyUrl(url) {
        // Accept URLs with various Spotify patterns
        const spotifyPatterns = [
            /^https?:\/\/open\.spotify\.com\/(track|playlist|album|artist)\/[a-zA-Z0-9]+/,
            /^https?:\/\/open\.spotify\.com\/playlist\/[a-zA-Z0-9]+\?si=[a-zA-Z0-9_-]+/,
    ...
    // UPDATED: Better Spotify URI extraction
    extractSpotifyUri(url) {
        // Clean the URL first
        const cleanUrl = url.trim();
        
        // Format 1: Regular Spotify URL (with or without query parameters)
        const patterns = [
        ...
        for (const pattern of patterns) {
            ...
            if (match) {
                ...
                console.log(`FocusFlow: Extracted Spotify ${type} ID: ${match[1]}`);
                return `${type}/${match[1]}`;
            }
        }
        
        // Format 2: Spotify URI (spotify:playlist:xxx)
        const uriPattern = /spotify:(track|playlist|album|artist):([a-zA-Z0-9]+)/;
        const uriMatch = cleanUrl.match(uriPattern);
        if (uriMatch) {
            console.log(`FocusFlow: Extracted Spotify URI: ${uriMatch[1]}/${uriMatch[2]}`);
            return `${uriMatch[1]}/${uriMatch[2]}`;
        }
        
        // Try to manually extract playlist ID from complex URLs
        if (cleanUrl.includes('open.spotify.com/playlist/')) {
            const parts = cleanUrl.split('/playlist/');
            if (parts.length > 1) {
                const idPart = parts[1].split('?')[0]; // Remove query parameters
                const playlistId = idPart.split('/')[0]; // Get just the ID
                if (playlistId && playlistId.length > 5) {
                    console.log(`FocusFlow: Manually extracted playlist ID: ${playlistId}`);
                    return `playlist/${playlistId}`;
                }
            }
        }
        
        console.log('FocusFlow: Could not extract Spotify URI from:', cleanUrl);
        return null;
    },
